...
| `messenger` | Yes | `inbound_token`, `messenger_signature` | `X-Hub-Signature-256` | `NOVAADAPT_CHANNEL_MESSENGER_APP_SECRET`, `NOVAADAPT_CHANNEL_MESSENGER_REQUIRE_SIGNATURE` | Uses Meta app-secret HMAC on raw body for page webhook events. |
| `instagram` | Yes | `inbound_token`, `instagram_signature` | `X-Hub-Signature-256` | `NOVAADAPT_CHANNEL_INSTAGRAM_APP_SECRET`, `NOVAADAPT_CHANNEL_INSTAGRAM_REQUIRE_SIGNATURE` | Uses Meta app-secret HMAC on raw body for Instagram webhook events. |
