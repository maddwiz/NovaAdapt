...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="XREAL X1 wearable intent bridge for NovaAdapt")
    parser.add_argument(
        "--bridge-url",
    ...
    parser.add_argument(
        ...
        default=(
            ...
            or os.getenv("NOVAADAPT_BRIDGE_SESSION_SUBJECT", "xreal-bridge")
        ),
    ...
    parser.add_argument("--source", default="xreal", help="Intent source tag (xreal, xreal_x1, etc.)")
    parser.add_argument(
        "--device-model",
        default=os.getenv("NOVAADAPT_XREAL_DEVICE_MODEL", "xreal-x1"),
        help="XREAL device model metadata (default: xreal-x1)",
    )
    parser.add_argument(
        "--display-mode",
    ...
    parser.add_argument(
        ...
        help="Optional XREAL firmware version for diagnostics metadata",
    )
    parser.add_argument(
        "--hand-tracking",
        action="store_true",
        default=_env_bool("NOVAADAPT_XREAL_HAND_TRACKING", False),
        help="Include hand-tracking enabled metadata in submitted objective",
    )
    parser.add_argument(
        "--submission-mode",
    ...
    parser.add_argument("--idempotency-prefix", default="xreal", help="Idempotency key prefix")
    return parser
...
def _build_runtime_client(
    ...
    if admin:
        ...
        issued = admin_client.issue_session_token(
            ...
            subject=(session_subject or "").strip() or "xreal-bridge",
...
def _build_submission_payload(intent: XRealIntent) -> dict[str, Any]:
    metadata: dict[str, Any] = {
        "source": intent.source,
        "confidence": intent.confidence,
        "intent_type": "wearable_voice",
        "wearable_family": "xreal",
        "device_model": intent.device_model,
        "display_mode": intent.display_mode,
        "hand_tracking": intent.hand_tracking,
        "captured_at": datetime.now(timezone.utc).isoformat(),
    }
    if intent.firmware_version:
        metadata["firmware_version"] = intent.firmware_version
    return {
...
def main() -> int:
    ...
    intent = XRealIntent(
        ...
        source=str(args.source).strip() or "xreal",
        device_model=str(args.device_model).strip() or "xreal-x1",
        display_mode=str(args.display_mode).strip() or "ar_overlay",
        firmware_version=str(args.firmware_version).strip(),
    ...
    try:
        payload = _submit_intent(
            ...
            idempotency_prefix=str(args.idempotency_prefix).strip() or "xreal",
        )
        ...
        output: dict[str, Any] = {
            ...
            "intent": {
                ...
                "device_model": intent.device_model,
                "display_mode": intent.display_mode,
                "firmware_version": intent.firmware_version or None,
                "hand_tracking": intent.hand_tracking,
            },
            **payload,
        }
